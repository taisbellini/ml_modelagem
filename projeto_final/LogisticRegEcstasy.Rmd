...
fullModLogEcstasy <- glm(ecstasy_user ~ Age+Gender+Education+NScore+CScore+AScore+EScore+OScore, data=trainLogEcstasy, weights = control, family = "binomial")
fullModLogEcstasy
...
predFullEcstasy
confusionMatrix(table(predFullEcstasy, testLogEcstasy$ecstasy_user), positive = "1")
```
...
confusionMatrix(table(predFullEcstasy03, testLogEcstasy$ecstasy_user), positive = "1")
...
```{r}
predStepEcstasy <- ifelse(predict(stepEcstasy, newdata = testLogEcstasy) >= 0.4, 1, 0)
confusionMatrix(table(predStepEcstasy, testLogEcstasy$ecstasy_user), positive="1")
