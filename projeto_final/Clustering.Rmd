...
data.to.cluster <- as.data.frame(data[, c("Age", "Education","Gender", "Nicotine")])
...
data.to.cluster$Nicotine <- as.factor(data.to.cluster$Nicotine)
```

KMODES: variacao do k-means para variaveis categoricas.
http://www.cs.ust.hk/~qyang/Teaching/537/Papers/huang98extensions.pdf
Começando com 2 clusters: 
...
```{r}
plot(km.data[,"Nicotine"], col=km$cluster)

clust <- km.data[,c("Nicotine","km$cluster")]
...
```{r}
km.3modes.data <- cbind(data.to.cluster, km.3modes$cluster)

plot(km.3modes.data[,"Nicotine"], col=km.3modes$cluster)

clust.3modes <- km.3modes.data[,c("Nicotine","km.3modes$cluster")]
...
#Análise de Clusters com k-modes focado em uma única droga - EXPERIMENTAL
