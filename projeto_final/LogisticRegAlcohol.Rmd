...
predFull <- ifelse(predict(fullModLogAlcohol, newdata = testLogAlcohol) >= 0.5, 1, 0)
confusionMatrix(table(predFull, testLogAlcohol$alcohol_user))
...
predStepAlcohol <- ifelse(predict(stepCannabis, newdata = testLogAlcohol) >= 0.5, 1, 0)
...
```{r}
predFilterAlcohol <- predict(fullModLogAlcogolFilter, testLogAlcohol)
