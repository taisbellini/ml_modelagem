...
Testando com ponto de corte em 0.5:
```{r}
predFullCannabis <- ifelse(predict(fullModLogCannabis, newdata = testLogCannabis) >= 0.5, 1, 0)
confusionMatrix(table(predFullCannabis, testLogCannabis$cannabis_user), positive = "1")
```
...
Observamos pelo gráfico que talvez seja interessante mover o ponto de corte para 0.4:

```{r}
predFullCannabis <- ifelse(predict(fullModLogCannabis, newdata = testLogCannabis) >= 0.4, 1, 0)
confusionMatrix(table(predFullCannabis, testLogCannabis$cannabis_user), positive = "1")
```
...
# Plot a probabilidade de cada individuo ser ou nao usuario e colorir de acordo com o verdadeiro
probCannabis <- ggplot(data=predicted.data, aes(x=rank, y=prob.user)) +
  geom_point(aes(color=user), alpha=1, shape=4, stroke=2) +
  xlab("Indivíduos ordenados") +
  ylab("Probabilidade predita de ser um usuário de Cannabis") +
  labs(color="Usuário") +
...
ggsave("LogRegCannabis.png", plot = probCannabis)
```

Testando com 0.35:
```{r}
predStepCannabis <- ifelse(predict(stepCannabis, newdata = testLogCannabis) >= 0.35, 1, 0)
confusionMatrix(table(predStepCannabis, testLogCannabis$cannabis_user), positive = "1")
