...
Observa-se que há uma relação linear positiva entre os dados e que o salário aumenta a medida que aumentam-se os anos de serviço. Portanto, consideramos apropriado utilizar a regressão linear.
...
------- | ----------------  |---------------- |------- |-------
Regressão | `r reg$coefficients[1]` | `r reg$coefficients[2]` | `r MAEin_reg` | `r MAEout_reg`
...
Observa-se que os valores ficaram muito próximos, não havendo diferença significativa entre os resultados dos dois métodos. 


Ao observarmos os dados, vemos que a distribuição de salário se aproxima de uma distribuição normal: 

```{r}
hist(data$Y, xlab= "salário", ylab="Frequência", main= "Histograma da variável salário")
```

Sabemos que o método MQO minimiza o ... 
...
**Gráficos ilustrando estatísticas dos coeficientes**

```{r graficos coeficientes}

boxplot(list("b0 Reg"=bhat_params$b0_reg, "b0 L1"=bhat_params$b0_l1))

boxplot(list("b1 Reg"=bhat_params$b1_reg, "b1 L1"=bhat_params$b1_l1))

```

Apesar de algumas diferenças na média dos coeficientes **b0** e **b1**, observamos que a média entre os erros absolutos médios fica muito próxima comparando ambos os métodos. Observamos também que o **MAEin** possui muito menos variabilidade do que o **MAEout** o que é esperado pois os coeficientes estão ajustados para o subconjunto de treinamento.

**Gráficos ilustrando estatísticas dos erros absolutos médios**

```{r graficos MAE}
boxplot(list("MAEin Reg"=MAE_table$MAEin_reg, "MAEin L1"=MAE_table$MAEin_l1,"MAEout Reg"=MAE_table$MAEout_reg, "MAEout L1"=MAE_table$MAEout_l1))
```
